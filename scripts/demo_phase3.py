...
def generate_synthetic_swimming_data(num_frames=100, fps=30.0):
    ...
    left_hand_path[:, 0] = (
        100 + 50 * np.sin(2 * np.pi * 0.5 * t) + np.random.normal(0, 2, num_frames)
    )
    left_hand_path[:, 1] = (
        200 + 30 * np.cos(2 * np.pi * 0.5 * t) + np.random.normal(0, 2, num_frames)
    )

    # Generate right hand path (similar but with slight asymmetry)
    right_hand_path = np.zeros((num_frames, 2))
    right_hand_path[:, 0] = (
        300 + 45 * np.sin(2 * np.pi * 0.5 * t + 0.2) + np.random.normal(0, 2, num_frames)
    )
    right_hand_path[:, 1] = (
        200 + 28 * np.cos(2 * np.pi * 0.5 * t + 0.2) + np.random.normal(0, 2, num_frames)
    )
    ...
    right_shoulder = (
        118 + 32 * np.cos(2 * np.pi * 0.5 * t + 0.2) + np.random.normal(0, 3, num_frames)
    )
...
def demo_smoothing():
    ...
    true_trajectory = np.column_stack([100 + 50 * np.sin(t), 200 + 30 * np.cos(t)])
    ...
    print(
        f"RMS noise (Kalman): {np.sqrt(np.mean((kalman_smoothed - true_trajectory)**2)):.2f} pixels"
    )
    print(
        f"RMS noise (Savitzky-Golay): {np.sqrt(np.mean((savgol_smoothed - true_trajectory)**2)):.2f} pixels"
    )
    print(
        f"RMS noise (Moving Average): {np.sqrt(np.mean((ma_smoothed - true_trajectory)**2)):.2f} pixels"
    )
    ...
    axes[0, 0].plot(
        noisy_trajectory[:, 0], noisy_trajectory[:, 1], "o", alpha=0.5, label="Noisy", markersize=4
    )
    ...
    axes[0, 1].plot(
        noisy_trajectory[:, 0], noisy_trajectory[:, 1], "o", alpha=0.3, label="Noisy", markersize=4
    )
    ...
    axes[1, 0].plot(
        savgol_smoothed[:, 0], savgol_smoothed[:, 1], "b-", linewidth=2, label="Savitzky-Golay"
    )
...
def demo_feature_extraction():
    ...
    if "right_hand_mean_velocity" in features:
        print(
            f"🚀 Right Hand Mean Velocity: {features['right_hand_mean_velocity']:.2f} pixels/frame"
        )
    ...
    axes[0, 0].plot(
        data["left_hand_path"][:, 0],
        data["left_hand_path"][:, 1],
        "b-",
        linewidth=2,
        label="Left Hand",
    )
    axes[0, 0].plot(
        data["right_hand_path"][:, 0],
        data["right_hand_path"][:, 1],
        "r-",
        linewidth=2,
        label="Right Hand",
    )
    ...
    categories = {
        "Temporal": len(
            [k for k in features if any(x in k for x in ["stroke_rate", "cycle", "tempo"])]
        ),
        "Kinematic": len(
            [k for k in features if any(x in k for x in ["velocity", "acceleration", "path"])]
        ),
        "Angular": len(
            [
                k
                for k in features
                if any(x in k for x in ["elbow", "shoulder"]) and "asymmetry" not in k
            ]
        ),
        "Symmetry": len([k for k in features if "asymmetry" in k]),
        "Injury Risk": len(
            [k for k in features if any(x in k for x in ["extreme", "drop", "risk", "workload"])]
        ),
    }

    axes[1, 1].bar(
        categories.keys(), categories.values(), color=["blue", "green", "orange", "purple", "red"]
    )
